// import db from "../config.js"; // Your database config

// // --- Quiz Setup Endpoints ---

// /**
//  * GET /quizzes
//  * Fetches all quizzes for the dashboard.
//  */
...
// /**
//  * POST /quizzes
//  * Creates a new quiz setup.
//  * Body: { name: "My New Quiz" }
//  */
...
//       "INSERT INTO quiz_setup(name) VALUES ($1) RETURNING *", 
//       [name]
//     );
//     res.status(201).json(result.rows[0]); // Return the created quiz
//   } catch (err) {
//     console.error("Error creating quiz:", err);
//     // Handle case where name might not be unique
//     if (err.code === '23505') { // unique_violation
//         return res.status(409).json({ error: "A quiz with this name already exists." });
//     }
...
// /**
//  * PUT /quizzes/:name/timer
//  * Updates the date, time, and duration for a specific quiz.
//  * Body: { quizDate, quizTime, quizDuration }
//  */
...
// /**
//  * GET /quizzes/:quizName/questions
...
// /**
//  * POST /quizzes/:quizName/questions
...
//       (question_id, question, options1, options2, options3, options4, answer, description, image, quizname, marks, negativeMarks, questiontype) 
...
/** GET /api/quizzes - Fetches all quizzes for the dashboard. */
export const getAllQuizzes = async (req, res) => {
  try {
    const result = await db.query("SELECT id, name, time, date, duration FROM quiz_setup ORDER BY id DESC");
    res.status(200).json(result.rows);
